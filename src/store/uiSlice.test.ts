...
describe('uiSlice', () => {
  ...
  describe('savingIds management', () => {
    it('should add a saving ID', () => {
      ...
      expect(result.savingIds['campaign-1']).toBe(true)
    })
    ...
    it('should add multiple different saving IDs', () => {
      ...
      expect(state.savingIds).toEqual({ 'campaign-1': true, 'campaign-2': true })
    })

    it('should remove a saving ID', () => {
      const state: UIState = {
        ...initialState,
        savingIds: { 'campaign-1': true, 'campaign-2': true },
      }
      const action = removeSavingId('campaign-1')
      const result = uiReducer(state, action)
      expect(result.savingIds).toEqual({ 'campaign-2': true })
    })

    it('should handle removing non-existent saving ID', () => {
      const state: UIState = { ...initialState, savingIds: { 'campaign-1': true } }
      const action = removeSavingId('campaign-2')
      const result = uiReducer(state, action)
      expect(result.savingIds).toEqual({ 'campaign-1': true })
    })
  })
  ...
  describe('errorIds management', () => {
    it('should add an error ID', () => {
      ...
      const result = uiReducer(initialState, action)
      expect(result.errorIds['campaign-1']).toBe(true)
    })

    it('should remove an error ID', () => {
      const state: UIState = {
        ...initialState,
        errorIds: { 'campaign-1': true, 'campaign-2': true },
      }
      const action = removeErrorId('campaign-1')
      const result = uiReducer(state, action)
      expect(result.errorIds).toEqual({ 'campaign-2': true })
    })
